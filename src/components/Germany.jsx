...
const Germany = () => {
  ...
  return (
    ...
    <div className='room-three'>
        <h1>Germany</h1>
        <h2  className='length'> Number Of Student : {germanyUsers.length}</h2>
        ...
        onClick={() =>{dispatch(addUser({id :germanyUsers.length + 1 ,name : name , phone : phone , course : course , book :book , note : note}))
      setName("");
      setCourse("");
      setPhone("")
      }}
    ...
    {germanyUsers.length > 0 ?  <table border = '2' cellPadding = '10'>
        ...
        {germanyUsers.length > 0 && germanyUsers.filter((user) =>
