...
import { addUser, deleteUser } from '../redux/slices/RoomNineSlice';
...
const RoomNine = () => {
    ...
    const [paid , setPaid] = useState();
      ...
      const roomNineUsers = useSelector(state =>state.roomNine.users);
  ...
  <div className='english-room'>
        <h1>Room 9 English</h1>
        <h2  className='length'> Number Of Student : {roomNineUsers.length}</h2>
     ...
     {roomNineUsers.length > 0 ? <table border = '2' cellPadding = '10'>
        <tr>
            <th>id</th>
            <th>name</th>
            <th>Phone Number</th>
        ...
        {roomNineUsers.length > 0 && roomNineUsers.filter((user) =>
