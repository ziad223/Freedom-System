...
import { addUser, deleteUser } from '../redux/slices/RoomThreeSlice';
...
const RoomThreeE = () => {
  ...
  const [paid , setPaid] = useState();
    ...
    const roomThreeUsers = useSelector(state =>state.roomThree.users);
  ...
  <div className='english-room'>
      <h1>Room 3 English</h1>
      <h2  className='length'> Number Of Student : {roomThreeUsers.length}</h2>
        ...
        onClick={() =>{dispatch(addUser({
          id :roomThreeUsers.length + 1 ,name : name , phone : phone , course : course , note : note , book :book,paid : paid ,  residual : residual
        }))
      setName("");
      setCourse("");
      setPhone("");
     ...
     {roomThreeUsers.length > 0 ? <table border = '2' cellPadding = '10'>
        <tr>
            <th>id</th>
            <th>name</th>
            <th>Phone Number</th>
        ...
        {roomThreeUsers.length > 0 && roomThreeUsers.filter((user) =>
