...
import { addUser, deleteUser } from '../redux/slices/RoomFourSlice';
...
const RoomFourE = () => {
  ...
  const [paid , setPaid] = useState();
    ...
    const roomFourUsers = useSelector(state =>state.roomFour.users);
  ...
  <div className='english-room'>
      <h1>Room 4 English</h1>
      <h2  className='length'> Number Of Student : {roomFourUsers.length}</h2>
        ...
        onClick={() =>{dispatch(addUser({
          id :roomFourUsers.length + 1 ,name : name , phone : phone , course : course , note : note , book :book , paid : paid , residual : residual
        }))
      setName("");
      setCourse("");
      setPhone("");
     ...
     {roomFourUsers.length > 0 ? <table border = '2' cellPadding = '10'>
        <tr>
            <th>id</th>
            <th>name</th>
            <th>Phone Number</th>
        ...
        {roomFourUsers.length > 0 && roomFourUsers.filter((user) =>
