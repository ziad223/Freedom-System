...
const Icdl = () => {
  ...
  return (
   ...
   <Sidebar/>
    <div className='no-replay'>
        <h1>Icdl</h1>
        <h2  className='length'> Number Of Student : {icdlUsers.length}</h2>
  ...
  onClick={() =>{dispatch(addUser({id :icdlUsers.length + 1 ,name : name , phone : phone  , book : book , note : note}))
    setName("");
    setPhone("");
    setBook("");
    setNote("");
...
{icdlUsers.length > 0 ?  <table border = '2' cellPadding = '10'>
  ...
  {icdlUsers.length > 0 && icdlUsers.filter((user) =>
