...
import { addUser, deleteUser } from '../redux/slices/RoomFiveSlice';
...
const RoomFourE = () => {
  ...
  const [paid , setPaid] = useState();
    ...
    const roomFiveUsers = useSelector(state =>state.roomFive.users);
 ...
 <div className='english-room'>
      <h1>Room 5 English</h1>
      <h2  className='length'> Number Of Student : {roomFiveUsers.length}</h2>
        ...
        onClick={() =>{dispatch(addUser({
          id :roomFiveUsers.length + 1 ,name : name , phone : phone , course : course , note : note , book :book , paid : paid , residual : residual
        }))
      setName("");
      setCourse("");
      setPhone("");
     ...
     {roomFiveUsers.length > 0 ? <table border = '2' cellPadding = '10'>
        <tr>
            <th>id</th>
            <th>name</th>
            <th>Phone Number</th>
        ...
        {roomFiveUsers.length > 0 && roomFiveUsers.filter((user) =>
