...
import { addUser, deleteUser } from '../redux/slices/RoomEightSlice';
...
const RoomEight = () => {
    ...
    const [paid , setPaid] = useState();
      ...
      const roomEightUsers = useSelector(state =>state.roomEight.users);
  ...
  return (
   ...
   <div className='english-room'>
        <h1>Room 8 English</h1>
        <h2  className='length'> Number Of Student : {roomEightUsers.length}</h2>
     ...
     {roomEightUsers.length > 0 ? <table border = '2' cellPadding = '10'>
        <tr>
            <th>id</th>
            <th>name</th>
            <th>Phone Number</th>
        ...
        {roomEightUsers.length > 0 && roomEightUsers.filter((user) =>
