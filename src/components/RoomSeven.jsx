...
import { addUser, deleteUser } from '../redux/slices/RoomSevenSlice';
...
const RoomSeven = () => {
    ...
    const [paid , setPaid] = useState();
      ...
      const roomSevenUsers = useSelector(state =>state.roomSeven.users);
  ...
  return (
   ...
   <div className='english-room'>
        <h1>Room 7 English</h1>
        <h2  className='length'> Number Of Student : {roomSevenUsers.length}</h2>
     ...
     {roomSevenUsers.length > 0 ? <table border = '2' cellPadding = '10'>
        <tr>
            <th>id</th>
            <th>name</th>
            <th>Phone Number</th>
        ...
        {roomSevenUsers.length > 0 && roomSevenUsers.filter((user) =>
